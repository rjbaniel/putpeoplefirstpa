			...
			$filtered_forums = array();
				...
				foreach ($forum_list as $forum) {						
					...
					if(private_groups_can_user_view_post($user_id, $forum)) {
						//then it can be seen by private groups, but is user allowed to post in it with TOPIC PERMISSIONS?
						//first if topic permissions not switched on, then add to array
						if (empty ($rpg_topic_permissions['activate']) ) {
						array_push($filtered_forums, $forum);
						}else {
							$valuecheck = pg_check_topic_permissions ($forum) ;
							//so if check is null or 4 or 2 then user can create topics in this forum
								if (empty ($valuecheck) || !empty ($valuecheck['check4']) || !empty ($valuecheck['check2']) ) {
									array_push($filtered_forums, $forum);
								}
						}
					}
